...
def load_samples(input_paths):
    ...
    with tf.name_scope('input_images'):
        input_images = crop_image(inputs, seed)

    with tf.name_scope('target_images'):
        target_images = crop_image(targets, seed)
...
def crop_image(image, seed):
    return tf.image.crop_to_bounding_box(image, 0, 0, image_size, image_size)
